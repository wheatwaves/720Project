...
def load_data(data_path):
    data = scipy.io.loadmat(open(data_path))
    return [data['X']/255.0,data['y']]

def fetch_data(X,idx,batch_size):
    return X[:,:,:,batch_size*idx:batch_size*(idx+1)]
...
# Y_list = [0]*train_iters
# for i in range(train_iters):
#     print i
#     xtrain,ytrain = train_data.next_batch(batch_size)
#     feed_dict = {x:xtrain}
#     features = sess.run(hidden_feature,feed_dict)

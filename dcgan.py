...
def plot_loss(losses, epoch):
    ...
    plt.legend()
    plt.savefig('figs/loss_' + str(epoch) + '.png')
...
def plot_gen(epoch, n_ex=16, dim=(4,4), figsize=(10,10)):
    ...
    for i in range(generated_images.shape[0]):
        ...
        plt.axis('off')
    plt.tight_layout()
    plt.savefig('figs/gen_' + str(epoch) + '.png')
...
train_for_n(nb_epoch=num_epoch_1, plt_frq=500, BATCH_SIZE=32)

# Train for 2000 epochs at reduced learning rates
opt.lr.set_value(1e-5)
dopt.lr.set_value(1e-4)
train_for_n(start_epoch=num_epoch_1, nb_epoch=num_epoch_2, plt_frq=500, BATCH_SIZE=32)

# Train for 2000 epochs at reduced learning rates
opt.lr.set_value(1e-6)
dopt.lr.set_value(1e-5)
train_for_n(start_epoch=num_epoch_1 + num_epoch_2, nb_epoch=num_epoch_2, plt_frq=500, BATCH_SIZE=32)
